...
train_0<-train[train$QuoteConversion_Flag==0,]
train_1<-train[train$QuoteConversion_Flag==1,]

for(x in 1:5) {
  dat_1<-sample(nrow(train_1),40000)
  dat_0<-sample(nrow(train_0),40000)
  
  train2<-data.frame(rbind(train_1[dat_1,],train_0[dat_0,]))
  tra<-train2[,feature.names]
  h<-sample(nrow(tra),800)
  ...
  CV<-xgb.cv(param, dtrain, 400, nfold=5,
         , showsd = FALSE)
...
submission <- data.frame(QuoteNumber=test$QuoteNumber, QuoteConversion_Flag=(listapred[,2]/5)) 
cat("saving the submission file\n")
write_csv(submission, "uBoos_xgboostV1.2.csv")
